# bindings/python/numrus/num_array_class.py
import itertools
import math
...
class NumArray:
    ...
    inner: Union[
        _numrus.PyNumArrayF32, _numrus.PyNumArrayF64, _numrus.PyNumArrayU8
    ]
    dtype: str
    ...
    def __init__(
        ...
        if isinstance(data, NumArray):
            ...
            self.inner = data
            if isinstance(data, _numrus.PyNumArrayF32):
                self.dtype = "float32"
            elif isinstance(data, _numrus.PyNumArrayF64):
                self.dtype = "float64"
            elif isinstance(data, _numrus.PyNumArrayU8):
                self.dtype = "uint8"
            else:
                self.dtype = "unknown"
        else:
            ...
            if self._is_nested_list(data):
                ...
                if dtype == "float32":
                    self.inner = _numrus.PyNumArrayF32(flat_data, shape)
                elif dtype == "float64":
                    self.inner = _numrus.PyNumArrayF64(flat_data, shape)
                elif dtype == "uint8":
                    self.inner = _numrus.PyNumArrayU8(flat_data, shape)
                ...
                if dtype == "float32":
                    self.inner = _numrus.PyNumArrayF32(flat_data)
                elif dtype == "float64":
                    self.inner = _numrus.PyNumArrayF64(flat_data)
                elif dtype == "uint8":
                    self.inner = _numrus.PyNumArrayU8(flat_data)
                elif dtype in ("int32", "int64"):
                    # Implement PyNumArrayInt32 and PyNumArrayInt64 similarly if needed
                    raise NotImplementedError(
                        f"dtype '{dtype}' is not yet implemented."
    ...
    def matmul(self, other: "NumArray") -> "NumArray":
        ...
        if self.dtype == "float32":
            ...
            result = _numrus.matmul_f32(self.inner, other.inner)
            ...
            result = _numrus.matmul_f64(self.inner, other.inner)
        else:
    ...
    def dot(self, other: "NumArray") -> "NumArray":
        ...
        if self.dtype == "float32":
            result = _numrus.dot_f32(self.inner, other.inner)
        elif self.dtype == "float64":
            result = _numrus.dot_f64(self.inner, other.inner)
        else:
    ...
    def mean(
        ...
        result = (
            _numrus.mean_f32(self.inner, axis_list)
            if self.dtype == "float32"
            else _numrus.mean_f64(self.inner, axis_list)
        )
        return NumArray(result, dtype=self.dtype)
    ...
    def median(
        ...
        axis_list = [axis] if isinstance(axis, int) else axis
        result = (
            _numrus.median_f32(self.inner, axis_list)
            if self.dtype == "float32"
            else _numrus.median_f64(self.inner, axis_list)
        )
        return NumArray(result, dtype=self.dtype)
    ...
    def min(
        ...
        if axis is None:
            return (
                _numrus.min_f32(self.inner)
                if self.dtype == "float32"
                else _numrus.min_f64(self.inner)
            )

        axis_list = [axis] if isinstance(axis, int) else axis
        result = (
            _numrus.min_axis_f32(self.inner, axis_list)
            if self.dtype == "float32"
            else _numrus.min_axis_f64(self.inner, axis_list)
        )
        return NumArray(result, dtype=self.dtype)
    ...
    def max(
        ...
        if axis is None:
            return (
                _numrus.max_f32(self.inner)
                if self.dtype == "float32"
                else _numrus.max_f64(self.inner)
            )

        axis_list = [axis] if isinstance(axis, int) else axis
        result = (
            _numrus.max_axis_f32(self.inner, axis_list)
            if self.dtype == "float32"
            else _numrus.max_axis_f64(self.inner, axis_list)
        )
        return NumArray(result, dtype=self.dtype)
    ...
    def concatenate(self, other: "NumArray", axis: int) -> "NumArray":
        ...
        if self.dtype == "float32":
            result = _numrus.concatenate_f32([self.inner, other.inner], axis)
        elif self.dtype == "float64":
            result = _numrus.concatenate_f64([self.inner, other.inner], axis)
        else:
    ...
    def norm(
        ...
        if self.dtype == "float32":
            return NumArray(
                _numrus.norm_f32(self.inner, p, axis, keepdims), dtype="float32"
            )
        elif self.dtype == "float64":
            return NumArray(
                _numrus.norm_f64(self.inner, p, axis, keepdims), dtype="float64"
            )
        else:
