...
def zeros(shape: List[int], dtype: str = "float32") -> NumArray:
    ...
    """
    if dtype == "float32":
        return NumArray(_numrus.zeros_f32(shape), dtype=dtype)
    elif dtype == "float64":
        return NumArray(_numrus.zeros_f64(shape), dtype=dtype)
    else:
...
def ones(shape: List[int], dtype: str = "float32") -> NumArray:
    ...
    """
    if dtype == "float32":
        return NumArray(_numrus.ones_f32(shape), dtype=dtype)
    elif dtype == "float64":
        return NumArray(_numrus.ones_f64(shape), dtype=dtype)
    else:
        raise ValueError(f"Unsupported dtype: {dtype}")
...
def arange(
    ...
    """
    if dtype == "float32":
        return NumArray(_numrus.arange_f32(start, stop, step), dtype=dtype)
    elif dtype == "float64":
        return NumArray(_numrus.arange_f64(start, stop, step), dtype=dtype)
    else:
        raise ValueError(f"Unsupported dtype: {dtype}")
...
def linspace(start: float, stop: float, num: int, dtype: str = "float32") -> NumArray:
    ...
    """
    if dtype == "float32":
        return NumArray(_numrus.linspace_f32(start, stop, num), dtype=dtype)
    elif dtype == "float64":
        return NumArray(_numrus.linspace_f64(start, stop, num), dtype=dtype)
    else:
        raise ValueError(f"Unsupported dtype: {dtype}")
...
def concatenate(arrays: List[NumArray], axis: int = 0) -> NumArray:
    ...
    if arrays[0].dtype == "float32":
        return NumArray(
            _numrus.concatenate_f32([a.inner for a in arrays], axis), dtype="float32"
        )
    elif arrays[0].dtype == "float64":
        return NumArray(
            _numrus.concatenate_f64([a.inner for a in arrays], axis), dtype="float64"
        )
    else:
...
def norm(
    ...
    """
    if a.dtype == "float32":
        return NumArray(_numrus.norm_f32(a.inner, p, axis, keepdims), dtype="float32")
    elif a.dtype == "float64":
        return NumArray(_numrus.norm_f64(a.inner, p, axis, keepdims), dtype="float64")
    else:
...
def hamming_distance(a: bytes, b: bytes) -> int:
    ...
    return _numrus.hamming_distance(list(a), list(b))
...
def hamming_batch(
    ...
    return _numrus.hamming_batch(list(query), list(database), num_rows, row_bytes)
...
def hamming_top_k(
    ...
    return _numrus.hamming_top_k(list(query), list(database), num_rows, row_bytes, k)


def bundle_u8(vectors: List[NumArray]) -> NumArray:
    """Majority-vote bundle of multiple u8 NumArray vectors (VSA superposition)."""
    return NumArray(_numrus.bundle_u8([v.inner for v in vectors]), dtype="uint8")
...
def quantize_f32_to_u8(data: List[float]) -> Tuple[List[int], float, int]:
    ...
    return _numrus.quantize_f32_to_u8(data)
...
def quantize_f32_to_i8(data: List[float]) -> Tuple[List[int], float, int]:
    ...
    return _numrus.quantize_f32_to_i8(data)
...
def int8_gemm_i32(
    ...
    return _numrus.int8_gemm_i32(a, b, m, n, k)
...
def int8_gemm_f32(
    ...
    return _numrus.int8_gemm_f32(a, b, m, n, k, scale_a, zero_point_a, scale_b)
